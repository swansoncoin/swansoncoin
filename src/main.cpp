...
uint256 hashGenesisBlock("0x74f5fb19a735f76ad3a2ad567a916aefd38a45b38523d8e2d7318b47edae22b0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SwansonCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SwansonCoin Signed Message:\n";
...
{
    // SwansonCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight == 1)							//PREMINE %2
	{
		nSubsidy = 14835.52 * COIN;
	} 
	else if (nHeight % 1000 == 0) 				//BONUS BLOCK EVERY 1000 BLOCKS
	{
		if (rand >= 800) {
			nSubsidy = 5 * COIN;
		} else if (rand >= 600) {
		...
		if (nHeight <= 241919) {
			if (rand >= 900) {
				...
				nSubsidy = 0.16666666 * COIN;
			} else if (rand >= 600) {
				nSubsidy = 0.125 * COIN;
			} else if (rand >= 500) {
				nSubsidy = 0.1 * COIN;
			} else if (rand >= 400) {
				nSubsidy = 0.08333333 * COIN;
			} else if (rand >= 300) {
				nSubsidy = 0.07142857 * COIN;
			} else if (rand >= 200) {
				nSubsidy = 0.0625 * COIN;
			} else if (rand >= 100) {
				nSubsidy = 0.05555555 * COIN;
			} else if (rand >= 1) {
				nSubsidy = 0.05 * COIN;
			}
		}	
		if (nHeight >= 241920) {				//6 MONTH HALVING
			if (rand >= 900) {
				nSubsidy = 0.25 * COIN;
			} else if (rand >= 800) {
				nSubsidy = 0.125 * COIN;
				...
				nSubsidy = 0.04166666 * COIN;
			} else if (rand >= 300) {
				nSubsidy = 0.03571428 * COIN;
			} else if (rand >= 200) {
				nSubsidy = 0.03125 * COIN;
			} else if (rand >= 100) {
				nSubsidy = 0.02777777 * COIN;
			} else if (rand >= 1) {
				nSubsidy = 0.025 * COIN;
			}	
		}
...
static const int64 nTargetTimespan = 10 * 60; // 10 minutes - 10 blocks
static const int64 nTargetSpacing = 1 * 60; // 1 minutes		-- 1440 blocks a day - 40320 blocks a month
...
unsigned int static GetNextWorkRequired_DigiShield(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
		...
		else if        (DiffMode == 3) { return GetNextWorkRequired_DigiShield(pindexLast, pblock); } // DigiShield
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SwansonCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // SwansonCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "SwansonCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1392436295;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1655045;

        if (fTestNet)
        {
            block.nTime    = 1392436310;
            block.nNonce   = 0;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x902ff0848c13d9b552dfb44168f1f8f194262644fea09d20ea81df5f32b5f78f"));
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0x88 }; // SwansonCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SwansonCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SwansonCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SwansonCoinMiner(CWallet *pwallet)
{
    printf("SwansonCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("swansoncoin-miner");
    ...
    try { loop {
        ...
        printf("Running SwansonCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SwansonCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SwansonCoinMiner, pwallet));
}
